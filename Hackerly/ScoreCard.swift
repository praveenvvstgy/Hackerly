...
class ScoreCard: UIButton {
    ...
    @IBInspectable
    var borderNotFavoriteColor: UIColor = UIColor(hex:"9e9b9b") {
        didSet {
            updateLayerProperties()
        }
    }
...
//        print(self.superview?.bounds)
    ...
    private func createLayersIfNeeded() {
        if fillRingShape == nil {
            ...
            fillRingShape.fillColor = UIColor.clearColor().CGColor
            fillRingShape.strokeColor = favoriteColor.CGColor
...
//            fillRingShape.transform = CATransform3DMakeScale(0.2, 0.2, 0.2)
        ...
        if outerRingShape == nil {
            ...
            outerRingShape.bounds = CGPathGetBoundingBox(outerRingShape.path)
            outerRingShape.lineWidth = lineWidth
            outerRingShape.strokeColor = borderNotFavoriteColor.CGColor
            outerRingShape.fillColor = UIColor.clearColor().CGColor
            outerRingShape.position = CGPoint(x: CGRectGetWidth(outerRingShape.bounds)/2, y: CGRectGetHeight(outerRingShape.bounds)/2)
            outerRingShape.transform = CATransform3DIdentity
            outerRingShape.opacity = 1
            self.layer.addSublayer(outerRingShape)
        }
        if starShape == nil {
            var starFrame = frameWithInset()
            starFrame.size.width = CGRectGetWidth(starFrame)/2
            starFrame.size.height = CGRectGetHeight(starFrame)/2
            ...
            starShape.fillColor = notFavoriteColor.CGColor
            starShape.position = CGPoint(x: CGRectGetWidth(CGPathGetBoundingBox(outerRingShape.path))/2, y: CGRectGetHeight(CGPathGetBoundingBox(outerRingShape.path))/2 - 1)
    ...
    private func frameWithInset() -> CGRect {
        return CGRectInset(CGRect(x: 0, y: 0, width: 40, height: 35), lineWidth/2, lineWidth/2)
    }
